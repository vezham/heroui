...
export function extractHeroUIImport(code: string): string[] {
  ...
  // Filter the lines to only include import statements from "@v0xoss/react"
  const importStatements = lines.filter(
    (line) => line.startsWith("import") && line.includes("@v0xoss/react"),
  );
...
export function updateTailwindConfig(tailwindConfig: string, componentNames: string[]): string {
  ...
  const lineIndex = lines.findIndex((line) =>
    line.includes("./node_modules/@v0xoss/theme/dist/**/*.{js,ts,jsx,tsx}"),
  );
  ...
  if (lineIndex !== -1) {
    ...
    componentNames.forEach((componentName) => {
      ...
      // Add the new line
      lines.splice(lineIndex, 0, `  "./node_modules/@v0xoss/theme/dist/components/${name}.js",`);
    });
  }

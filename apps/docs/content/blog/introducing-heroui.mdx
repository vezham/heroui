...
- All `@nextui-org` packages are now deprecated and moved to the `@v0xoss` organization
  - Example: `@nextui-org/react` → `@v0xoss/react`
- No new bug fixes or features will be released to the `@nextui-org` packages

We recommend migrating to the new `@v0xoss` packages as soon as possible. To ensure a smooth transition, we haven't introduced any breaking changes or new features with this rebranding.

...
> If you are using it from `@vx-oss/react`, no changes are required.

The new `cn` utility exported from the `@vx-oss/theme` package includes `tailwind-merge` to avoid conflicts between tailwindcss classes overrides and includes the config for NextUI custom classes.

If you are using the `cn` utility from the `@vx-oss/system`, 

```diff-tsx
-import {cn} from "@vx-oss/system"
```

or `@vx-oss/system-rsc` package,

```diff-tsx
-import {cn} from "@vx-oss/system-rsc"
```

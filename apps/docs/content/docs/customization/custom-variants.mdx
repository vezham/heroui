...
Let's use the [Button](/docs/components/button) component as an example. It's a non-slots component, and you can view its source styles [here](https://github.com/vezham/heroui/blob/main/packages/core/theme/src/components/button.ts).
...
```tsx
// MyButton.tsx
import {extendVariants, Button} from "@vx-oss/react";
...
The `extendVariants` function can also be used with slot-based components like [Input](/docs/components/input) to add or override variants. You can view the Input component's source styles [here](https://github.com/vezham/heroui/blob/main/packages/core/theme/src/components/input.ts).
...
```tsx
// MyInput.tsx
import {extendVariants, Input} from "@vx-oss/react";
